...
//	SetGraphMode(1280, 720, 32);
	...
	//charamob[0].anim.type[SPEEDMODE]			= MV1DuplicateModel(Original[0].type[SPEEDMODE]);
	player[2].anim.typestop[SPEEDMODE]		= MV1DuplicateModel(Original[0].typestop[SPEEDMODE]);
	player[2].anim.type[BALANCEMODE]		= MV1DuplicateModel(Original[0].type[BALANCEMODE]);
	player[2].anim.typestop[BALANCEMODE]	= MV1DuplicateModel(Original[0].typestop[BALANCEMODE]);
	player[2].anim.type[POWERMODE]			= MV1DuplicateModel(Original[0].type[POWERMODE]);
	player[2].anim.typestop[POWERMODE]		= MV1DuplicateModel(Original[0].typestop[POWERMODE]);

	for ( int i = 3 ; i < 12 ; i++ )
	{
		if ((i >= 3) && (i <= 5)) {
			player[i].anim.type[SPEEDMODE]		= MV1DuplicateModel(Original[0].type[SPEEDMODE]);
			player[i].anim.typestop[SPEEDMODE]	= MV1DuplicateModel(Original[0].typestop[SPEEDMODE]);			
		}
		if ((i >= 6) &&(i <= 8)) {
			player[i].anim.type[BALANCEMODE]	= MV1DuplicateModel(Original[0].type[BALANCEMODE]);
			player[i].anim.typestop[BALANCEMODE]= MV1DuplicateModel(Original[0].typestop[BALANCEMODE]);
		}
		if ((i >= 9)&&(i <= 11)) {
			player[i].anim.type[POWERMODE]		= MV1DuplicateModel(Original[0].type[POWERMODE]);
			player[i].anim.typestop[POWERMODE]	= MV1DuplicateModel(Original[0].typestop[POWERMODE]);
		}		
	}
...
*/				break;
			...
			case eScenePlay :
				...
				GamemodeChenge_flg = 1;
					...
					/*if (player[0].GetPosition().y >= 18800) {
					gamemode = eSceneClear;
					}*/
				// デバック用
				// 通常の描画結果を描画する
			
					if ((player[0].GetPosition().y >= 4000) || (player[1].GetPosition().y >= 4000)) {
						GamemodeChenge_flg = 1;
						gamemode = eSceneClear;
						// BMP画像の表示
						//左から第一陣						
							if (player[0].GetPosition().y > player[1].GetPosition().y) {
								player[0].SetPosition(VGet(1600.0f, 250.0f, 1000.0f));
								player[0].SetDiection(DOWN);
								MV1SetPosition(player[0].anim.model, player[0].GetPosition());
								MV1DrawModel(player[0].anim.model);
								player[1].SetPosition(VGet(2300.0f, 100.0f, 1000.0f));
							}
							else {
								player[1].SetPosition(VGet(1600.0f, 250.0f, 1000.0f));
								MV1SetPosition(player[1].anim.model, player[1].GetPosition());
								MV1DrawModel(player[1].anim.model);
								player[0].SetPosition(VGet(1000.0f, 100.0f, 1000.0f));
							}
							if (GamemodeChenge_flg == 1) {								
								DrawGraph(-1000 + x, 0, BmpDate[0], TRUE);
								DrawGraph(-1000 + x, 360, BmpDate[1], TRUE);
			...
			case eSceneClear :
				...
				player[0].ChangeAnimation(g_Chara[0], player[0].anim.stop);
				g_Chara[0]->CharaRizarut(g_Chara[0]);
			
				player[1].ChangeAnimation(g_Chara[1], player[1].anim.stop);
				g_Chara[1]->CharaRizarut(g_Chara[1]);
				...
				// プレイヤー
				player[2].SetPosition(VGet(1100.0f, 200.0f, 1100.0f));
				player[3].SetPosition(VGet(1300.0f, 100.0f, 1300.0f));
				player[4].SetPosition(VGet(1500.0f, 100.0f, 1500.0f));
				player[5].SetPosition(VGet(1700.0f, 100.0f, 1700.0f));
				player[6].SetPosition(VGet(1900.0f, 100.0f, 1900.0f));
				player[7].SetPosition(VGet(2100.0f, 100.0f, 1900.0f));
				player[8].SetPosition(VGet(2300.0f, 100.0f, 1700.0f));
				player[9].SetPosition(VGet(2500.0f, 100.0f, 1500.0f));
				player[10].SetPosition(VGet(2700.0f, 100.0f, 1300.0f));
				player[11].SetPosition(VGet(2900.0f, 100.0f, 1100.0f));

				MV1SetPosition(player[0].anim.model, player[0].GetPosition());
				MV1SetPosition(player[1].anim.model, player[1].GetPosition());
				MV1SetPosition(player[2].anim.model, player[2].GetPosition());
				MV1DrawModel(player[0].anim.model);
				MV1DrawModel(player[1].anim.model);
				...
				for ( int i = 3 ; i < 12 ; i++ )
				{
					MV1SetPosition(player[i].anim.model, player[i].GetPosition());
